...
//switch case?
   searchFunction = async(event) => {
        ...
        console.log('call res is set', this.state.filteredData)
       ...
       await this.setState({filteredData: results});
       console.log('agency is set', this.state.filteredData)
        }   else if (this.state.selection === 'brief_description'){
            console.log('brief desc is set', this.state.filteredData)
    ...
    onChange = async (event) => {
        event.preventDefault();
        console.log("Select list event: " + event.target.value);       
        await this.setState({
    ...
    if(filteredData.length >= 1 && filteredData[num] !== undefined)  {
        return (
            ...
            <nav><Link to="/App.js">Clear</Link></nav>
        ...
        return(
            ...
            <nav style={{marginTop: 80}}><Link to="/App.js">Clear</Link></nav>
            <Route exact path="/App.js" component={App} />
